...
class record_hint extends external_api {
    ...
    public static function execute_parameters() {
        return new external_function_parameters([
            ...
            'level' => new external_value(PARAM_INT, 'Hint level'),
     ...
     * Record hint usage
     */
    public static function execute($problemid, $level, $sesskey) {
        global $DB, $USER, $PAGE;
        ...
        $attempt = $DB->get_record('aicode_attempts',
            ['problemid' => $params['problemid'], 'userid' => $USER->id],
            '*', IGNORE_MULTIPLE);

        if ($attempt) {
            $hints = json_decode($attempt->used_hints_json ?? '[]', true);
            $hints[] = ['level' => $params['level'], 'time' => time()];
