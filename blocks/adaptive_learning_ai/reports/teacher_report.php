...
use block_adaptive_learning_ai\report_helper;
...
$students = report_helper::get_course_students($courseid);
$student_labels = [];
$student_data = [];
...
foreach ($students as $student) {
    ...
    $average = report_helper::get_student_average_score($courseid, $student->id);
...
if ($userid) {
    ...
    $attempts = report_helper::get_student_quiz_attempts($courseid, $userid);
    ...
    if (empty($attempts)) {
        ...
        foreach ($attempts as $attempt) {
            if ($attempt->state === 'finished' && $attempt->sumgrades !== null) {
                ...
                $quiz_scores[] = report_helper::calculate_percentage($attempt->sumgrades, $attempt->max_grade);
            }
        }
        ...
        foreach ($sorted_attempts as $attempt) {
            $score_pct  = report_helper::calculate_percentage($attempt->sumgrades, $attempt->max_grade);
            ...
            $level_summary_rows = '';
            foreach ($sorted_attempts as $attempt) {
                $pct = report_helper::calculate_percentage($attempt->sumgrades, $attempt->max_grade);
                if ($pct >= 90) {
        ...
        foreach ($attempts as $attempt) {
            $score = $attempt->sumgrades !== null ? report_helper::calculate_percentage($attempt->sumgrades, $attempt->max_grade) . '%' : '-';
            $status = report_helper::get_attempt_status($attempt->state);
            $date = report_helper::format_time($attempt->timefinish ?: $attempt->timestart);
        ...
        if ($selectedattempt) {
            ...
            $responses = report_helper::get_question_responses($selectedattempt->id);
            ...
            if (empty($responses)) {
                ...
                foreach ($responses as $response) {
                    $detailtable->data[] = [format_text($response->questiontext, FORMAT_HTML), s($response->responsesummary), s($response->rightanswer), report_helper::calculate_percentage($response->fraction, 1) . '%'];
                }
    ...
    foreach ($students as $student) {
        $scoreinfo = report_helper::get_student_score_info($courseid, $student->id);
        $average = report_helper::get_student_average_score($courseid, $student->id);
        ...
        if ($scoreinfo && $scoreinfo->sumgrades !== null && $scoreinfo->maxgrade !== null) {
            $percentage = report_helper::calculate_percentage($scoreinfo->sumgrades, $scoreinfo->maxgrade);
            $scoretxt = $percentage . '%';
            $status = report_helper::get_attempt_status($scoreinfo->state);
        }

        if ($average !== null) {
            $averagetxt = $average . '%';
            $leveltxt = report_helper::get_score_level_text($average, 100);
        }
