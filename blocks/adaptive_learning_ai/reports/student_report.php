...
// ── AMBIL DATA ATTEMPTS ──────────────────────────────────────────────
$attempts        = report_helper::get_student_quiz_attempts($courseid, $USER->id);
...
if (empty($attempts)) {
    ...
    if (!empty($finished_attempts)) {
        $scores    = array_map(fn($a) => report_helper::calculate_percentage($a->sumgrades, $a->max_grade), $finished_attempts);
    ...
    foreach ($attempts as $attempt) {
        $pct    = $attempt->sumgrades !== null
                  ? report_helper::calculate_percentage($attempt->sumgrades, $attempt->max_grade)
                  : null;
        $score  = $pct !== null ? $pct . '%' : '-';
    ...
    if ($selectedattempt) {
        ...
        if (empty($responses)) {
            ...
            foreach ($responses as $response) {
                $pct_q     = report_helper::calculate_percentage($response->fraction, 1);
