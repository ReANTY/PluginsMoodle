...
class report_helper {
    ...
    public static function get_students_quiz_list($courseid) {
        ...
        $sql = "SELECT DISTINCT 
                    ...
                    COALESCE(NULLIF(q.sumgrades, 0), q.grade, 100) as max_grade
                FROM {user} u
    ...
    public static function get_quiz_attempt_detail($attemptid) {
        ...
        $sql = "SELECT qa.id, qa.quiz, qa.userid, qa.attempt, qa.sumgrades, qa.timefinish, qa.timestart, qa.state,
                       q.id as quizid, q.name as quiz_name,
                       COALESCE(NULLIF(q.sumgrades, 0), q.grade, 100) as max_grade,
    ...
    public static function get_student_average_score($courseid, $userid) {
        ...
        $sql = "SELECT AVG(CASE WHEN COALESCE(NULLIF(q.sumgrades, 0), q.grade, 0) > 0 
                                THEN (qa.sumgrades / COALESCE(NULLIF(q.sumgrades, 0), q.grade, 100)) * 100 
    ...
    public static function get_student_score_info($courseid, $userid) {
        ...
        $sql = "SELECT qa.sumgrades,
                       COALESCE(NULLIF(q.sumgrades, 0), q.grade, 100) as maxgrade,
                       qa.state, qa.timefinish, qa.timestart
                FROM {quiz_attempts} qa
                JOIN {quiz} q ON qa.quiz = q.id
                WHERE q.course = ? AND qa.userid = ? AND qa.state = 'finished'
                ORDER BY qa.timefinish DESC
...
class_alias(report_helper::class, 'adaptive_learning_ai_report_helper');
