...
$pageurl = new moodle_url('/local/llmmotivation/learner_profile_detail.php', [
    'userid'   => $userid,
    'courseid' => $courseid,
]);

$listurl = new moodle_url('/local/llmmotivation/learner_profiles.php', [
    'courseid' => $courseid,
]);
...
$PAGE->set_title(get_string('learnerprofiledetail', 'local_llmmotivation'));
$PAGE->set_heading($course->fullname);
$PAGE->set_pagelayout('incourse');

// Breadcrumb navigation.
$PAGE->navbar->add(get_string('pluginname', 'local_llmmotivation'));
$PAGE->navbar->add(
    get_string('learnerprofilemonitoring', 'local_llmmotivation'),
    $listurl
);
$PAGE->navbar->add(
    get_string('learnerprofiledetail', 'local_llmmotivation'),
    $pageurl
);
...
function get_level_label(int $level): string {
    switch ($level) {
        case 1: return get_string('low',    'local_llmmotivation');
        case 2: return get_string('middle', 'local_llmmotivation');
        case 3: return get_string('high',   'local_llmmotivation');
        default: return '—';
    }
}
...
if ($profile) {
    ...
    $template_data = [
        ...
        'noprofilefound'   => get_string('noprofilefound', 'local_llmmotivation'),
    ];
}
...
echo $OUTPUT->header();
echo $OUTPUT->heading(
    get_string('learnerprofiledetail', 'local_llmmotivation') . ': ' . $learner_fullname
);

echo $OUTPUT->render_from_template(
    'local_llmmotivation/learner_profile_detail',
    $template_data
);
