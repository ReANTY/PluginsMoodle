...
use local_llmmotivation\output\analytics_dashboard_page;
use local_llmmotivation\admin\config_audit_log;
use local_llmmotivation\admin\config_audit_renderer;
...
$PAGE->set_url(new moodle_url('/local/llmmotivation/analytics_dashboard.php'));
$PAGE->set_title(get_string('analytics_dashboard_title', 'local_llmmotivation'));
$PAGE->set_heading(get_string('analytics_dashboard_title', 'local_llmmotivation'));
...
$PAGE->navbar->add(get_string('analytics_dashboard', 'local_llmmotivation'));
...
foreach ($category_map as $cat_id => $meta) {
    ...
    $performance_distribution[] = [
        'category_name' => get_string($meta['string'], 'local_llmmotivation'),
        'count'         => $count,
...
echo html_writer::div(
    html_writer::tag('p',
        get_string('manage_resources_link_desc', 'local_llmmotivation'),
...
echo html_writer::div(
    html_writer::tag('p',
        get_string('viewlearnerprofiles_desc', 'local_llmmotivation'),
        ['class' => 'text-muted mb-0']
    ),
    'mt-3 p-3 border rounded bg-light'
);
...
echo html_writer::tag('h3',
    get_string('audit_log_title', 'local_llmmotivation'),
    ['class' => 'mt-4']
);

echo html_writer::tag('p',
    get_string('audit_log_desc', 'local_llmmotivation'),
    ['class' => 'text-muted']
);
...
echo html_writer::div(
    html_writer::link(
        new moodle_url('/local/llmmotivation/config_audit_log.php'),
        get_string('audit_view_full_log', 'local_llmmotivation'),
