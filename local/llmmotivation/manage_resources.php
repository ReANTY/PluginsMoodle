...
use local_llmmotivation\repository\learning_resource_repository;
use local_llmmotivation\form\resource_form;
...
$base_url = new moodle_url('/local/llmmotivation/manage_resources.php', ['courseid' => $courseid]);

$PAGE->set_context($context);
$PAGE->set_url($base_url);
$PAGE->set_course($course);
$PAGE->set_title(get_string('manage_resources_title', 'local_llmmotivation'));
$PAGE->set_heading($course->fullname);
$PAGE->set_pagelayout('incourse');

// Breadcrumb navigation.
$PAGE->navbar->add(
    get_string('pluginname', 'local_llmmotivation')
);
$PAGE->navbar->add(
    get_string('manage_resources_title', 'local_llmmotivation'),
    $base_url
);
...
if ($action === 'delete' && $id > 0) {
    ...
    if (!$resource) {
        redirect($base_url, get_string('resource_not_found', 'local_llmmotivation'), null, \core\output\notification::NOTIFY_ERROR);
    }

    if ($confirm) {
        // Perform soft-delete (sets is_active = 0).
        $repo->delete_resource($id);
        redirect($base_url, get_string('resource_deleted', 'local_llmmotivation'), null, \core\output\notification::NOTIFY_SUCCESS);
    }

    // Show confirmation page.
    $PAGE->navbar->add(get_string('deleteresource', 'local_llmmotivation'));

    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string('deleteresource', 'local_llmmotivation'));
    ...
    echo $OUTPUT->confirm(
        get_string('resource_delete_confirm', 'local_llmmotivation', format_string($resource->title)),
        $confirm_url,
        $base_url
    );
...
if ($action === 'add' || $action === 'edit') {
    ...
    if ($form_data_submitted = $mform->get_data()) {
        ...
        if ($action === 'edit' && $id > 0) {
            ...
            redirect($base_url, get_string('resource_updated', 'local_llmmotivation'), null, \core\output\notification::NOTIFY_SUCCESS);
        } else {
            ...
            redirect($base_url, get_string('resource_added', 'local_llmmotivation'), null, \core\output\notification::NOTIFY_SUCCESS);
        }
    }

    // Render form.
    $heading_key = ($action === 'edit') ? 'editresource' : 'addresource';
    $PAGE->navbar->add(get_string($heading_key, 'local_llmmotivation'));

    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string($heading_key, 'local_llmmotivation'));
    $mform->display();
    echo $OUTPUT->footer();
    exit;
}
...
foreach ($resources_raw as $r) {
    ...
    $type_label = isset($type_label_keys[$type_key])
        ? get_string($type_label_keys[$type_key], 'local_llmmotivation')
        : ucfirst($type_key);
    ...
    $resources_data[] = [
        ...
        'is_active_label'     => $r->is_active
            ? get_string('resource_active', 'local_llmmotivation')
            : get_string('resource_inactive', 'local_llmmotivation'),
        'is_active_class'     => $r->is_active ? 'success' : 'secondary',
        'edit_url'            => $edit_url->out(false),
        'delete_url'          => $delete_url->out(false),
        'delete_confirm'      => get_string('resource_delete_confirm', 'local_llmmotivation', format_string($r->title)),
    ];
}
...
$template_data = [
    ...
    'sesskey'          => sesskey(),
    'no_resources_msg' => get_string('no_resources', 'local_llmmotivation'),
];
...
/** @var \local_llmmotivation\output\renderer $renderer */
$renderer = $PAGE->get_renderer('local_llmmotivation');

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('manage_resources_title', 'local_llmmotivation'));
