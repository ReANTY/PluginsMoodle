...
/**
 * Configuration settings validator for local_llmmotivation (ACMLS).
 *
...
class settings_validator {
    ...
    public static function validate_llm_provider($value) {
        ...
        if (!in_array($value, $allowed, true)) {
            return get_string('error_validation_llm_provider', 'local_llmmotivation');
        }
    ...
    public static function validate_api_key($value) {
        if (!is_string($value) || trim($value) === '') {
            return get_string('error_validation_api_key', 'local_llmmotivation');
        }
    ...
    public static function validate_ollama_endpoint($value) {
        if (!is_string($value) || trim($value) === '') {
            return get_string('error_validation_ollama_endpoint', 'local_llmmotivation');
        }
    ...
    public static function validate_leaderboard_weights($w1, $w2, $w3) {
        ...
        foreach ($weights as $weight) {
            if (!is_numeric($weight)) {
                return get_string('error_validation_weight_range', 'local_llmmotivation');
            }
    ...
    public static function validate_motivation_threshold($value) {
        if (!is_numeric($value)) {
            return get_string('error_validation_motivation_threshold', 'local_llmmotivation');
        }
    ...
    public static function validate_learning_rate($value) {
        if (!is_numeric($value)) {
            return get_string('error_validation_learning_rate', 'local_llmmotivation');
        }
    ...
    public static function validate_decline_threshold($value) {
        if (!is_numeric($value)) {
            return get_string('error_validation_decline_threshold', 'local_llmmotivation');
        }
