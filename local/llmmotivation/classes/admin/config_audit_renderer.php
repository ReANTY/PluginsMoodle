...
namespace local_llmmotivation\admin;
...
class config_audit_renderer {
    ...
    public static function render_table(
        ...
        if (empty($entries)) {
            return \html_writer::div(
                \html_writer::tag(
                    'p',
                    get_string('audit_no_entries', 'local_llmmotivation'),
                    ['class' => 'text-muted']
                ),
                'alert alert-info'
            );
        ...
        // Build table header.
        $header_cells = [
            \html_writer::tag('th', get_string('audit_col_time', 'local_llmmotivation'),
                ['scope' => 'col']),
            \html_writer::tag('th', get_string('audit_col_user', 'local_llmmotivation'),
                ['scope' => 'col']),
            \html_writer::tag('th', get_string('audit_col_setting', 'local_llmmotivation'),
                ['scope' => 'col']),
            \html_writer::tag('th', get_string('audit_col_old_value', 'local_llmmotivation'),
                ['scope' => 'col']),
            \html_writer::tag('th', get_string('audit_col_new_value', 'local_llmmotivation'),
                ['scope' => 'col']),
        ];
        ...
        if ($total > $perpage) {
            ...
            $output .= \html_writer::tag(
                'p',
                get_string('audit_showing', 'local_llmmotivation',
            ...
            $output .= \html_writer::tag(
                'p',
                get_string('audit_total_entries', 'local_llmmotivation', $total),
                ['class' => 'text-muted small']
            );
        }
    ...
    public static function render_summary_card(int $count): string {
        $title   = get_string('audit_log_title', 'local_llmmotivation');
        $label   = get_string('audit_recent_changes', 'local_llmmotivation');
