...
namespace local_llmmotivation\profiling;

defined('MOODLE_INTERNAL') || die();

use local_llmmotivation\tracking\activity_log;
...
class profiling_system {
    ...
    public function __construct() {
        $configured = get_config('local_llmmotivation', 'learning_rate');
    ...
    public function notify_coach(learner_profile $profile): void {
        if (class_exists('\local_llmmotivation\coach\coach')) {
            try {
                \local_llmmotivation\coach\coach::receive_profile_update($profile);
            } catch (\Exception $e) {
                debugging(
